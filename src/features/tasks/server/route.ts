...
const app = new Hono()
    ...
    .post(
        ...
        async (c) => {
            ...
            const heightPositionTask = await databases.listDocuments(
                TASKS_ID,
                DATABASE_ID,
                [
                    Query.equal("status", status),
                    Query.equal("workspaceId", workspaceId),
                    Query.orderAsc("positioon"),
                    Query.limit(1),
                ],
            );

            const newPosition = 
                heightPositionTask.documents.length >0
                ? heightPositionTask.documents[0].position + 1000
                : 1000;
